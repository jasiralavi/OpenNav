...
use gtk4::prelude::*;
use gtk4::{
    Align, ApplicationWindow, Box as GtkBox, Label, ListView, Orientation, ScrolledWindow,
    SignalListItemFactory, SingleSelection, StringFilter, StringList,
};
...
pub fn show_profile_picker(
    ...
    if profiles.is_empty() {
        let dialog = gtk4::MessageDialog::builder()
            ...
            .secondary_text(
                "Open Google Chrome once and make sure its profiles exist in ~/.config/google-chrome.",
            )
            .build();
    ...
    let launch_selected: std::rc::Rc<dyn Fn(bool)> = {
        ...
        std::rc::Rc::new(move |keep_open| {
            ...
            let Ok(string_object) = item.downcast::<gtk4::StringObject>() else {
                return;
            };
            let name = string_object.string();
            let Some(profile) = profiles.iter().find(|p| p.name == name.as_str()) else {
                return;
            };

            let _ =
                browser_repository::launch_chrome_profile(&browser_id, &profile.directory, &url);
